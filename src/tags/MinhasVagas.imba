...
export default tag MinhasVagas
	...
	def doAdd
		window.location.assign('/#/criar/vaga')

	def doRemove id
		loading = true
		render()
		await fb.excluir_vaga(id)
		mount()
	...
	def render
		<self>
			if loading
				...
				<h4.offset-sm-2.mb-3.mt-5> "MINHAS VAGAS"
				<div.card.col-sm-8.offset-sm-2.mb-5.p-0>
					<div.card-header>
						<h6.m-0.d-flex.align-items-center.justify-content-between> "ADICIONAR VAGA"
							<i.icon.material-icons title="Adicionar" @click=doAdd> "add"
					for vaga in vagas
						if vaga.id_empresa == user_id
							<div.card.m-3>
								<div.card-header>
									<h6.m-0.d-flex.align-items-center.justify-content-between> "{vaga.titulo_da_vaga.toUpperCase()}"
										<div>
											<i.icon.material-icons title="Editar" @click=(do doEdit(vaga.id))> "edit"
											<i.icon.material-icons title="Excluir" @click=(do doRemove(vaga.id))> "delete"
								<div.card-body.pb-0>
									<p.m-0> vaga.salario
