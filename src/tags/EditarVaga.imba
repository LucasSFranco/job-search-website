...
export default tag EditarVaga
	...
	def mount
		...
		vaga = await fb.firestore.collection('vagas').doc(id).get()
		vaga = vaga.data()
		loading = false
	...
	def create
		...
		loading = true
		id = window.location.hash.slice(9)
		await fb.firestore.collection("vagas").doc(id).set(vaga)
	...
	def render
		<self>
			if loading
				...
				<h4.offset-sm-3.mb-3.mt-5> "EDITAR VAGA"
					...
					<div.tab-content>
						...
						<div.tab-pane.fade #informacoes_para_pessoas_com_deficiencia>
							...
							<div.card-footer.d-flex.justify-content-between>
								...
								<button.btn.btn-primary type="button" @click=create> "Alterar"